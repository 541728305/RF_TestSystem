...
using FCT_TestSystem.SerialPort;
...
namespace RF_TestSystem
{
    ...
    public partial class RF_TestSystem : Form
    {
        ...
        //等待FCT测试完成标志
        System.Timers.Timer waitFctTestTimer = new System.Timers.Timer(100);//实例化样本计时Timer类，设置间隔时间为1000毫秒；
        int waitingFctSeconds = 0;
        bool fctTesting = false;
        bool getFctResult = false;

        //结果显示
        DataTable itemTable = new DataTable();

        //工时
        System.Timers.Timer testTimesTimer = new System.Timers.Timer(100);//实例化Timer类，设置间隔时间为10000毫秒；
        ...
        public delegate void SerialPortDataReceivedHandle(string receivedBuffer); //处理串口接受数据
        public event SerialPortDataReceivedHandle SerialPortDataReceivedEvent;
        #endregion
        ...
        public void SystemInit()
        {
         ...
         //   setLimitToChart(Gloable.myTraces);
            ...
            mySerialPort.ReceiveEvent += MySerialPort_ReceiveEvent;
            SerialProtocol.SL_ITE_1_Itemname_Event += SerialProtocol_SL_ITE_1_Itemname_Event;
            SerialProtocol.SL_DAT_1_Event += SerialProtocol_SL_DAT_1_Event;
            SerialProtocol.SN_Event += getBarcode;
            SerialProtocol.STOP_TEST_Event += SerialProtocol_STOP_TEST_Event;
            SerialProtocol.SCAN_Event += fctScanThread;

            //等待FCT测试定时器
            waitFctTestTimer.Elapsed += WaitFctTestTimer_Elapsed;
            waitFctTestTimer.AutoReset = true;

            //测试工时
            testTimesTimer.Elapsed += new System.Timers.ElapsedEventHandler(theout);//到达时间的时候执行事件；
            testTimesTimer.AutoReset = true;//设置是执行一次（false）还是一直执行(true)；
            //testTimesTimer.Enabled = true;//是否执行System.Timers.Timer.Elapsed事件；
            //初始化 FctDataGridView
            initFctDataGridView();
        }
        ...
        public void initStateHead()
        {
            ...
            Gloable.sateHead.uploadOracle = "UploadOracle";
        ...
        public void setSystemStateLabel(string state)
        {
            ...
            switch (state)
            {
                ...
                case "Free":
                    ...
                    this.systemStateTextBox.Text = "Waiting";
                ...
                case "Running":
                    ...
                    return;
                case "UploadOracle":
                    this.systemStateTextBox.Text = Gloable.sateHead.uploadOracle;
                    //Gloable.runningState.SystemSate = Gloable.sateHead.scan;
                    this.systemStateTextBox.BackColor = Color.Yellow;
                    return;
                default:
        ...
        private void startButton_Click(object sender, EventArgs e)
        {
            ...
            if(fctTesting == false)
            {
                setSystemStateLabel(Gloable.sateHead.testing);//空闲释放  
                string errorMsg = string.Empty;
                if (Gloable.modelSetting.openListEnable == true.ToString())
                {
                    if (mySerialPort.send(SerialProtocol.sendCommands.OP_TES_1, ref errorMsg))
                    {
                        clearfctDataGridViewResult();
                        fctTesting = true;
                        getFctResult = false;
                        waitFctTestTimer.Start();
                    }
                }
                else
                {
                    if (mySerialPort.send(SerialProtocol.sendCommands.HO_TES_1, ref errorMsg))
                    {
                        clearfctDataGridViewResult();
                        fctTesting = true;
                        getFctResult = false;
        ...
        private void setModelButton_Click(object sender, EventArgs e)
        {
            ...
            if( this.analysisModelComboBox.Text.ToString().Contains(Gloable.testInfo.currentModel))
            {
        ...
        SerialPortHelper mySerialPort = new SerialPortHelper();
        ...
        private void debugButton_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    mySerialPort.setPortName("COM4");


            //    if (!mySerialPort.open(ref exception))
            //    {
            ...
            //else
            //{
            //    if (!mySerialPort.close(ref exception))
            //    {
            //        MessageBox.Show(exception);
            //    }
            //    mySerialPort.ReceiveEvent -= MySerialPort_ReceiveEvent;
        ...
        private void debugButton2_Click(object sender, EventArgs e)
        {
            ...
            if (!mySerialPort.send(SerialProtocol.sendCommands.HO_ITE_1, ref exception))
            {
                MessageBox.Show(exception);
            }
        }
        ...
        private void openListSwitch_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void mainTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (this.mainTabControl.SelectedIndex != this.mainTabControl.TabPages.Count - 1)
            {
                ...
                if(checkModelSettingChange(Gloable.modelSetting, getModelSettingFromDataTable()) == false)
                {
        ...
        public void fctScanThread(string comm)
        {
            if (scanning == false && systemTesting == false && systemStart == true)
            {
                Thread scanThread = new Thread(scanBarcodeEvent);
        ...
        public ModelSetting getModelSettingFromDataTable()
        {
            ...
            modelSetting.openListEnable = this.openListSwitch.Checked.ToString();
            return modelSetting;
        }
        ...
        public void setModelSettingToDataTable(ModelSetting modelSetting)
        {
            ...
            this.openListSwitch.Checked = Convert.ToBoolean(modelSetting.openListEnable);
            try 
            {
        ...
        public void writeInfoTextBox(string text)
        {
            ...
            this.testInfoTextBox.AppendText(infoStringHead() + text + "    TextSize:" + this.testInfoTextBox.Text.Length / 8 + "Byte" + "\r\n");
        ...
        public bool saveConfig()
        {
            ...
            if (successful == true)
            {
                ...
                if (serialConnected == true)
                {
                    ...
                    if (Gloable.modelSetting.openListEnable == true.ToString())
                    {
                        mySerialPort.send(SerialProtocol.sendCommands.OP_ITE_1, ref errorMsg);
                    }
                    else
                    {
                        mySerialPort.send(SerialProtocol.sendCommands.HO_ITE_1, ref errorMsg);
                    }
                }
        ...
        private bool serialConnect()
        {
            foreach (string portName in SerialPort.GetPortNames())
            {
                ...
                if(! mySerialPort.open(ref errorMsg))
                {
                    writeInfoTextBox(errorMsg);
                ...
                if (!mySerialPort.waitFromSend(SerialProtocol.sendCommands.HO_CNC_1, SerialProtocol.receiveCommands.SL_CNC_1OK, 500, ref errorMsg))
                {
                    mySerialPort.close(ref errorMsg);
                }
                else
                {
        ...
        private void MySerialPort_ReceiveEvent(string receiveData)
        {
            SerialProtocol.commandParsing(receiveData);
            this.Invoke(new Action(() => { TCPRecordTextBox.AppendText(receiveData + "\r\n"); }));
        }

        /// <summary>
        /// 初始化table界面
        /// </summary>
        private void initFctDataGridView()
        {

            // FCT dataGridView画面优化
            Type fctType = this.fctDataGridView.GetType();
            System.Reflection.PropertyInfo fctpi = fctType.GetProperty("DoubleBuffered",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
            fctpi.SetValue(fctDataGridView, true, null);
        }
        ...
        private void SerialProtocol_SL_ITE_1_Itemname_Event(string command)
        {
            try
            {              
                itemTable.Dispose();
                itemTable = new DataTable();
                itemTable.Columns.Add("序号");
                itemTable.Columns.Add("项目");
                itemTable.Columns.Add("上限");
                itemTable.Columns.Add("下限");
                itemTable.Columns.Add("单位");
                itemTable.Columns.Add("类型");
                itemTable.Columns.Add("值");
                itemTable.Columns.Add("状态");

                List<string> itemString = new List<string>();
                List<string> upLimitString = new List<string>();
                List<string> downLimitString = new List<string>();
                ...
                {                  
                    ...
                    {   
                        try
                        {
                            Gloable.fctResult.itemNumber = Convert.ToInt32(item[0]);
                        }
                        catch
                        {
                            Gloable.fctResult.itemNumber = 0;
                        }
                        continue;
                    }

                    DataRow dataRow = itemTable.NewRow();
                    dataRow["序号"] = (i - 1).ToString();

                    for (int itemIndex = 0; itemIndex < item.Length; itemIndex++)
                    {
                        Console.WriteLine(item[itemIndex]);
                        dataRow[itemIndex + 1] = item[itemIndex];
                    }
                    itemString.Add(item[0]);
                    upLimitString.Add(item[1]);
                    downLimitString.Add(item[2]);
                    unit.Add(item[3]);
                    itemTable.Rows.Add(dataRow);

                }

                if(itemString.Count != Gloable.fctResult.ItemString.Count)
                {
                    createNewDataPath = true;
                    Gloable.fctResult.ItemString = itemString;
                    Gloable.fctResult.UpLimitString = upLimitString;
                    Gloable.fctResult.DownLimitString = downLimitString;
                    Gloable.fctResult.Unit = unit;
                    initFctModelFailCount(Gloable.fctResult.itemNumber);
                }
                else
                {
                    for (int i = 0; i < itemString.Count; i++)
                    {
                        if (Gloable.fctResult.ItemString[i] != itemString[i])
                        {
                            createNewDataPath = true;
                            Gloable.fctResult.ItemString = itemString;
                            Gloable.fctResult.UpLimitString = upLimitString;
                ...
                this.Invoke(new Action(() =>
                {
                    ...
                    this.fctDataGridView.DataSource = itemTable;
                    this.fctDataGridView.Columns[0].Width = 60;
                    this.fctDataGridView.Columns[1].Width = 250;
                    this.fctDataGridView.Columns[2].Width = 80;
                    this.fctDataGridView.Columns[3].Width = 80;
                    this.fctDataGridView.Columns[4].Width = 80;
                    this.fctDataGridView.Columns[5].Width = 80;
                    this.fctDataGridView.Columns[6].Width = 80; 
                    this.fctDataGridView.Columns[6].Width = 120;
                    this.fctDataGridView.Columns[7].Width = 80;

                    this.fctDataGridView.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    this.fctDataGridView.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                }));
            }
            catch(Exception e)
            {
        ...
        /// <param name="command"></param>
        [Obsolete]
        private void SerialProtocol_SL_DAT_1_Event(string command)
        {
            try
            {              
                string ngItem = string.Empty;
                string ngItemValue = string.Empty;
                fctTesting = false;
                getFctResult = true;
                ...
                this.Invoke(new Action(() =>
                {
                    if (Gloable.cameraInfo.cameraModel == (Gloable.cameraInfo.cameramOffModelString))
                    {
                        ...
                        if (checkProbeLife() == false)
                        {
                            ...
                            {
                                setSystemStateLabel(Gloable.sateHead.free);//空闲释放 
                                setSystemStateLabel(Gloable.sateHead.erorr);//空闲释放  
                                Gloable.myBarcode.Clear();
                        ...
                        barcodeChecked = true;
                        Gloable.myBarcode.Add(this.barcodeTextBox.Text);
                ...
                */
                ...
                if (barcodeChecked == false && manualTest == false)
                {
                    clearfctDataGridViewResult();
                    manualTest = false;
                    return;
                ...
                Gloable.fctResult.ResultString.Clear();

                string[] resultStrings = command.Split('#');
                Gloable.fctResult.dataNumber = (resultStrings.Length - 2);
                if (Gloable.fctResult.dataNumber != Gloable.fctResult.itemNumber)
                {
                    this.Invoke(new Action(() =>
                    {
                        setSystemStateLabel(Gloable.sateHead.free);//空闲释放 
                        setSystemStateLabel(Gloable.sateHead.erorr);//空闲释放  
                        writeInfoTextBox("表头数目与结果数目不匹配！");
                        MessageBox.Show("表头数目与结果数目不匹配！请重新获取表头！");
                        Gloable.myBarcode.Clear();
                        this.barcodeTextBox.Clear();
                    }));
                    manualTest = false;
                ...
                this.Invoke(new Action(() =>
                {
                    //this.fctDataGridView.Visible = false;
                ...
                for (int i = 1; i < resultStrings.Length; i++)
                {
                    ...
                    {
                        if(resultStrings[i].Contains("FAIL"))
                        {
                            Gloable.fctResult.Result = "FAIL";
                        }
                        else if(resultStrings[i].Contains("PASS"))
                        {
                            Gloable.fctResult.Result = "PASS";
                        }
                        else
                        {
                            this.Invoke(new Action(() =>
                            {
                                setSystemStateLabel(Gloable.sateHead.free);//空闲释放 
                                setSystemStateLabel(Gloable.sateHead.erorr);//空闲释放  
                                writeInfoTextBox("无效的测试结果！");
                                MessageBox.Show("无效的测试结果！");
                                Gloable.myBarcode.Clear();
                                this.barcodeTextBox.Clear();
                            }));
                            manualTest = false;
                    ...
                    string[] result = resultStrings[i].Split('*');
                    if (result.Length > 1)
                    {
                        itemTable.Rows[i - 1][6] = result[1];
                        Gloable.fctResult.ResultString.Add(result[1]);
                    }
                    else
                    {
                        Gloable.fctResult.ResultString.Add("");
                    }
                    this.Invoke(new Action(() =>
                    {
                        if (result[0].Equals("0"))
                        {
                            this.fctDataGridView.Rows[i - 1].Cells[7].Value = "PASS";
                        }
                        else if (result[0].Equals("1"))
                        {
                            this.fctDataGridView.Rows[i - 1].Cells[7].Value = "FAIL";
                            if (result.Length > 1)
                            {
                                ngItem += this.fctDataGridView.Rows[i - 1].Cells[1].Value + " ";
                                ngItemValue += result[1] + " ";
                ...
                this.Invoke(new Action(() =>
                {
                    //this.fctDataGridView.Visible = true;

                }));

                //存储数据
                string errorMsg = string.Empty;
                mySerialPort.send(SerialProtocol.sendCommands.HO_DAT_1_OK, ref errorMsg);
                ...
                FtpCopyFile.WaitOne();
                bool saveOK = Gloable.myOutPutStream.saveFctData(Gloable.dataFilePath, Gloable.fctResult, dataSaveDate);
                FtpCopyFile.ReleaseMutex();
                ...
                if (!saveOK)
                {
                    this.Invoke(new Action(() =>
                    {
                        setSystemStateLabel(Gloable.sateHead.free);//空闲释放 
                        setSystemStateLabel(Gloable.sateHead.erorr);//空闲释放  
                        writeInfoTextBox("写入本地csv失败！");
                        MessageBox.Show("写入本地csv失败！\r\n请检查文件是否被占用！");
                    }));
                    Gloable.myBarcode.Clear();
                    this.barcodeTextBox.Clear();
                    manualTest = false;
                ...
                this.Invoke(new Action(() =>
                {
                    setSystemStateLabel(Gloable.sateHead.free);//空闲释放 
                    bool upload = true;
                    bool isSampleTest = sampleTestFlag;
                    if (!manualTest)
                    {
                        upload = uploadOracle(Gloable.fctResult.Result, ngItem, ngItemValue);
                    }                  
                    if(upload)
                    {
                        setSystemStateLabel(Gloable.fctResult.Result);

                        //样本测试不统计良率
                        if(!isSampleTest)
                        {
                            updateTestInfo(Gloable.fctResult.Result);
                            fctStatisticalFailTop3(failList);
                        }
                        checkYeild();
                    }
                    else
                    {
                        setSystemStateLabel(Gloable.sateHead.OracleFail);//空闲释放  
                    }                
                    Gloable.myBarcode.Clear();
                    this.barcodeTextBox.Clear();
                }));
        ...
        /// <param name="e"></param>
        private void WaitFctTestTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            waitingFctSeconds++;
            if (getFctResult)
            {
        ...
        /// </summary>
        /// <param name="command"></param>
        private void SerialProtocol_STOP_TEST_Event(string command)
        {
            this.Invoke(new Action(() =>
            {
                setSystemStateLabel(Gloable.sateHead.free);//空闲释放  
        ...
        /// 清除测试结果
        /// </summary>
        private void clearfctDataGridViewResult()
        {
            for(int i =0;i < this.fctDataGridView.Rows.Count-1;i++)
            {
                this.fctDataGridView.Rows[i].Cells[6].Value = "";
                this.fctDataGridView.Rows[i].Cells[7].Value = "";
                this.fctDataGridView.Rows[i].Cells[6].Style.BackColor = Color.White;
                this.fctDataGridView.Rows[i].Cells[7].Style.BackColor = Color.White;
            }
        }
        ...
        /// <returns></returns>
        [Obsolete]
        private bool uploadOracle(string result,string ngItem,string ngValue)
        {
            if (Gloable.testInfo.currentModel == Gloable.testInfo.sampleEntryModelString)
            {
                ...
                this.Invoke(new Action(() =>
                {
                    setSystemStateLabel(Gloable.sateHead.uploadOracle);//空闲释放  
                    dialogResult = MessageBox.Show("是否录入数据库？", "样本录入", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation);
            ...
            if (sampleTestFlag == true)
            {
                ...
                if (OracleUploadFlag == true && manualTest == false)
                {
                    if (oracleHelper.loginOracle(Gloable.upLoadInfo.sampleDB, Gloable.upLoadInfo.sampleID, Gloable.upLoadInfo.samplePW))
                    {
                        oracleHelper.insertData(Gloable.upLoadInfo.sampleTB, barsamrecPackage.getBaesamInfoComlumnPackge(), barsamrecPackage.getBarsamInfoPackge());
            ...
            if (OracleUploadFlag == true && manualTest == false)
            {
                this.Invoke(new Action(() =>
                {
                    setSystemStateLabel(Gloable.sateHead.uploadOracle);
                    writeInfoTextBox("Oracle数据上传");
                }));
                Thread.Sleep(20);
                OracleDataPackage oracleDataPackage = new OracleDataPackage
                ...
                #region -- 使用鹏鼎API上传 --
                                ...
                                try
                                {
                                    uploadSuccessful = abb.uploadABB(avaCheckBarcodeConfig2,
                                    "FCT",
                                    oracleDataPackage.BARCODE,
                                    oracleDataPackage.PARTNUM,
                                    oracleDataPackage.REVISION,
                                    oracleDataPackage.WORKNO,
                                    oracleDataPackage.LINEID,
                                    oracleDataPackage.OPERTOR,
                                    oracleDataPackage.BARCODE,
                                    oracleDataPackage.TRESULT,
                                    oracleDataPackage.TESTDATE,
                                    oracleDataPackage.TESTTIME,
                                    "", "", "", "", "", "", "", "", "", "",
                                    oracleDataPackage.SDATE,
                                    oracleDataPackage.STIME,
                                    oracleDataPackage.FPATH,
                                    Gloable.dataFilePath,
                                    oracleDataPackage.NG_CATEGORY,
                                    oracleDataPackage.NG_ITEM,
                                    oracleDataPackage.NG_ITEM_VAL,
                                    ref errMsg
                                    );
                                }
                                catch (Exception uploadErr)
                ...
                #region -- Oracle客户端上传 --
                if (OracleUploadFlag == true )
                {
                    Thread.Sleep(20);
                    if (oracleHelper.loginOracle(Gloable.upLoadInfo.oracleDB, Gloable.upLoadInfo.oracleID, Gloable.upLoadInfo.oraclePW))
                    {
                        Thread.Sleep(20);
                        uploadSuccessful = oracleHelper.insertData(Gloable.upLoadInfo.oracleTB, oracleDataPackage.getOracleColumnPackege(), oracleDataPackage.getOraclePackege());                                           
                    }
                    else
                    {
                        this.Invoke(new Action(() =>
        ...
        /// 统计前三项不良
        /// </summary>
        private void fctStatisticalFailTop3(List<int> failNumber)
        {
            if(failNumber.Count<1)
            {
                return;
            }
            if(Gloable.testInfo.currentModel == Gloable.testInfo.productionModelString)
            {
                setTop3(ref Gloable.testInfo.productionModel, failNumber[0]);            
                updateTop3(Gloable.testInfo.productionModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.retestModelString)
            {
                setTop3(ref Gloable.testInfo.retestModel, failNumber[0]);
                updateTop3(Gloable.testInfo.retestModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.buyoffModelString)
            {
                setTop3(ref Gloable.testInfo.buyoffModel, failNumber[0]);
                updateTop3(Gloable.testInfo.buyoffModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.ORTModelString)
            {
                setTop3(ref Gloable.testInfo.ORTModel, failNumber[0]);
                updateTop3(Gloable.testInfo.ORTModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.FAModelString)
            {
                setTop3(ref Gloable.testInfo.FAModel, failNumber[0]);
                updateTop3(Gloable.testInfo.FAModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.SortingModelString)
            {
                setTop3(ref Gloable.testInfo.SortingModel, failNumber[0]);
                updateTop3(Gloable.testInfo.SortingModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.sampleEntryModelString)
            {
                setTop3(ref Gloable.testInfo.sampleEntryModel, failNumber[0]);
                updateTop3(Gloable.testInfo.sampleEntryModel);
            }
            else if (Gloable.testInfo.currentModel == Gloable.testInfo.developerModelString)
            {
                setTop3(ref Gloable.testInfo.developerModel, failNumber[0]);
                updateTop3(Gloable.testInfo.developerModel);
            }
        }
        ...
        /// 累计Fail项目
        /// </summary>
        /// <param name="model">测试模式</param>
        /// <param name="failMap">项目位置</param>
        private void setTop3(ref ModeInfo model,int failMap)
        {
            try
            {
                model.fctFailCount[failMap] ++;
            }
            catch(Exception e)
            {
                MessageBox.Show(e.Message);
        ...
        private void updateTop3(ModeInfo model)
        {
            if(model.fctFailCount == null || model.fctFailCount.Count == 0)
            {
                return;
            }
            List<int> temp = new List<int>(model.fctFailCount);
            ...
            this.Invoke(new Action(() =>
            {
                ...
                //top3[i,0] 为表头位置，top3[i,1]为NG数量
                for (int i = 0; i < 3; i++)
                {
                    top3[i, 0] = temp.IndexOf(temp.Max());
                    top3[i, 1] = temp.Max();

                    if (top3[i, 0] >= 0)
                    {
                        temp[top3[i, 0]] = 0;
                        this.failTop3Chart.Series[0].Points.AddXY(Gloable.fctResult.ItemString[top3[i, 0]], top3[i, 1]);
                    }
        ...
        /// 初始化不同模式下的Fail表头数
        /// </summary>
        private void initFctModelFailCount(int count)
        {
            Gloable.testInfo.productionModel.fctFailCount = new List<int>();
            Gloable.testInfo.retestModel.fctFailCount = new List<int>();
            Gloable.testInfo.buyoffModel.fctFailCount = new List<int>();
            Gloable.testInfo.ORTModel.fctFailCount = new List<int>();
            Gloable.testInfo.FAModel.fctFailCount = new List<int>();
            Gloable.testInfo.SortingModel.fctFailCount = new List<int>();
            Gloable.testInfo.sampleEntryModel.fctFailCount = new List<int>();
            Gloable.testInfo.developerModel.fctFailCount = new List<int>();
            for (int i = 0; i < count; i++)
            {
                Gloable.testInfo.productionModel.fctFailCount.Add(0);
                Gloable.testInfo.retestModel.fctFailCount.Add(0);
                Gloable.testInfo.buyoffModel.fctFailCount.Add(0);
                Gloable.testInfo.ORTModel.fctFailCount.Add(0);
                Gloable.testInfo.FAModel.fctFailCount.Add(0);
                Gloable.testInfo.SortingModel.fctFailCount.Add(0);
                Gloable.testInfo.sampleEntryModel.fctFailCount.Add(0);
                Gloable.testInfo.developerModel.fctFailCount.Add(0);
        ...
        ///  鼠标悬停显示前三项不良
        /// </summary>
        private void TestYieldTextBox_MouseHover(object sender, EventArgs e)
        {
            //ToolTip toolTip = new ToolTip();
            //toolTip.SetToolTip(this.TestYieldTextBox, "前三项不良");
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void fctDataGridView_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex > -1 && this.fctDataGridView.Rows[e.RowIndex].Cells.Count >= 7)
            {
                if (this.fctDataGridView.Rows[e.RowIndex].Cells[7].Value != null)
                {
                    if (this.fctDataGridView.Rows[e.RowIndex].Cells[7].Value.ToString().Equals("FAIL"))
                    {
                        this.fctDataGridView.Rows[e.RowIndex].Cells[6].Style.BackColor = Color.Red;
                        this.fctDataGridView.Rows[e.RowIndex].Cells[7].Style.BackColor = Color.Red;
                    }
                    else if(this.fctDataGridView.Rows[e.RowIndex].Cells[7].Value.ToString().Equals("PASS"))
                    {
                        this.fctDataGridView.Rows[e.RowIndex].Cells[6].Style.BackColor = Color.LightSeaGreen;
                        this.fctDataGridView.Rows[e.RowIndex].Cells[7].Style.BackColor = Color.LightSeaGreen;
                    }
                }
            }
        }
...
#region - Oracle数据查询 -
        ...
        private void inquireOracle()
        {
            ...
            inquireDataGridView.DataSource = oracleHelper.queryData(Gloable.upLoadInfo.oracleTB, "BARCODE", this.inquireBarcodeTextBox.Text.Trim());
        }
              ...
              Console.WriteLine(subdirectories.Name);
                ...
                foreach (string file in filelist)
                {
                    ...
                    catch (Exception e1)
                    {
                        this.Invoke(new Action(() =>
                        {
                            writeInfoTextBox(file+" 上传失败！" + e1.Message);
                        }));
                    }                       
                }
                if (Directory.Exists(localPathCopy))
...
#region - 全局静态类 -
    ...
    static class Gloable  //静态类,类似于全局变量的效果
    {
        ...
        public static FctResult fctResult = new FctResult();
    }
     ...
     public class FctResult
    {
        ...
        public string acupoint = string.Empty;
        public List<string> ItemString = new List<string>();
        public List<string> ResultString = new List<string>();
        public List<string> UpLimitString = new List<string>();
        public List<string> DownLimitString = new List<string>();
